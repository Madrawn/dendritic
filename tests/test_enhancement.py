...
def test_layer_placement_strategies(simple_mlp):
    ...
    enhanced_selective = enhance_model_with_dendritic(
        simple_mlp,
        target_layers=["fc1"]
    )
    named_modules = [*enhanced_selective.named_modules()]
    fc1_replaced = any(isinstance(m, (DendriticLayer, DendriticStack)) 
                      for name, m in named_modules if "fc1" in name)
    fc2_not_replaced = all(not isinstance(m, (DendriticLayer, DendriticStack)) 
                          for name, m in named_modules if "fc2" in name)
    assert fc1_replaced
...
def test_enhancement_ratios(simple_mlp):
    ...
    if hasattr(enhanced_scale, "scale"):
        assert torch.allclose(enhanced_scale.scale, torch.tensor(0.1))
    elif hasattr(enhanced_scale, "layer1") and hasattr(enhanced_scale.layer1, "scale"):
        assert torch.allclose(enhanced_scale.layer1.scale, torch.tensor(0.1))
...
def test_selective_layer_enhancement(simple_mlp):
    ...
    enhanced = enhance_model_with_dendritic(
        simple_mlp,
        target_layers=["fc1", "fc3"]
    )
    
    replaced_layers = [
        name for name, m in enhanced.named_modules()
...
def test_layer_config_serialization(simple_mlp):
    ...
    if hasattr(dendritic_layer, "dropout"):
        assert dendritic_layer.dropout.p == 0.2
...
def test_extract_and_load_dendritic_state(simple_mlp):
    ...
    for (n1, p1), (n2, p2) in zip(
        enhanced.named_parameters(), 
        new_enhanced.named_parameters()
    ):
        if p1.requires_grad:
