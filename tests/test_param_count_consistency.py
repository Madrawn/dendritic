...
from dendritic.experiments.param_utils import count_parameters
...
def test_dendritic_param_count_matches_calculation(device):
    ...
    print(f"Dendritic - Expected: {expected}, Actual: {actual}, Rel diff: {rel_diff:.4f}")
    assert rel_diff < tolerance, f"Trainable param count mismatch: expected {expected}, got {actual}"
...
def test_lora_param_count_matches_calculation(device):
    ...
    config = FinetuningConfig()
    config.lora_target_modules = list(set(config.lora_target_modules))  # Remove duplicates
    ...
    assert abs(actual - expected) / max(actual, expected) < tolerance, f"LoRA trainable param count mismatch: expected {expected}, got {actual}"