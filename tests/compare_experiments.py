        ...
        try:
            with open(file_path, 'r') as f:
                ...
                row = {
                    ...
                    'best_ppl': data.get('metrics', {}).get('best_eval_ppl', float('nan')),
                    'baseline_ppl': data.get('metrics', {}).get('baseline_ppl', float('nan')),
                    'rel_improvement': data.get('metrics', {}).get('relative_improvement', float('nan')),
                    'train_time_min': data.get('resources', {}).get('total_time_min', float('nan')),
                    'trainable_params': data.get('resources', {}).get('trainable_params', 0),
                ...
                if pd.notna(row['baseline_ppl']) and pd.notna(row['final_ppl']):
                    ...
                    row['rel_improvement'] = row['abs_improvement'] / row['baseline_ppl']
                else:
...
def compare_experiments():
    ...
    display_df['Trainable Params'] = display_df['Trainable Params']//1e6  # Convert to millions
    display_df[['Time (min)', 'Final PPL', 'Best PPL', 'Baseline PPL', 'Δ PPL']] = \
        display_df[['Time (min)', 'Final PPL', 'Best PPL', 'Baseline PPL', 'Δ PPL']].round(2)
