...
def sample_results(sample_config):
    """Create sample ConfidenceExperimentResults for testing."""
    ...
    # Create confidence model results
    conf_result = ConfidenceTrainingResult(
    ...
    results = ConfidenceExperimentResults(
        ...
        training_time={"standard": 239.0, "confidence": 259.0},
...
def test_plot_loss_curves_with_results_object(sample_results, tmp_path):
    """Test plot_loss_curves with ConfidenceExperimentResults object."""
...
def test_plot_training_time_comparison(sample_results, tmp_path):
    ...
    ax = fig.axes[0]
    bars = ax.patches
    assert len(bars) == 2  # Standard and confidence bars
...
def test_generate_summary_statistics(sample_results):
    ...
    # Verify confidence model stats
    conf_stats = stats["confidence"]
    assert "final_eval_loss_mean" in conf_stats
    assert "final_eval_loss_std" in conf_stats
    ...
    try:
        ...
        assert "confidence" in stats_from_file
    finally:
...
def test_generate_summary_statistics_empty_results(sample_config):
    ...
    empty_results = ConfidenceExperimentResults(
        standard_model_results={},
        confidence_model_results={},
        config=sample_config,
        timestamp="2024-01-01_12:00:00",
...
class TestInternalPlottingFunctions:
    ...
    def test_plot_token_loss_comparison(self, sample_results, mock_axes):
        ...
        # Should call hist for both standard and confidence
    ...
    @pytest.mark.unit
    def test_plot_confidence_loss(self, sample_results, mock_axes):
        """Test _plot_confidence_loss function."""
        _plot_confidence_loss(sample_results, mock_axes)

        # Should plot confidence loss history
        mock_axes.plot.assert_called()
        mock_axes.set_title.assert_called_with("Confidence Loss Over Time")

    @pytest.mark.unit
    def test_plot_confidence_loss_empty(self, sample_config, mock_axes):
        """Test _plot_confidence_loss with empty data."""
        # Create results with no confidence loss data
        empty_results = ConfidenceExperimentResults(
            standard_model_results={},
            confidence_model_results={},
            config=sample_config,
        ...
        _plot_confidence_loss(empty_results, mock_axes)

        # Should display "No confidence loss data available" text
        mock_axes.text.assert_called()
        mock_axes.set_title.assert_called_with("Confidence Loss Over Time")
