...
class MockTokenizer:
    ...
    def __call__(
        ...
        **kwargs,
        ...
        token_lengths = [[len(word) for word in text.split()] for text in texts]
        tokenized_input_ids = {"input_ids": token_lengths}
        return MockBatchEncoding({**tokenized_input_ids, "data": tokenized_input_ids})
...
def test_batch_collation(alpaca_handler, sample_dataset_dict):
    ...
    dataloader = torch.utils.data.DataLoader(
        train_dataset, batch_size=2, collate_fn=lambda x: x
    )
    ...
    for item in batch:
        assert sum(item["attention_mask"]) == sum(
            1 for id in item["input_ids"] if id != alpaca_handler.tokenizer.pad_token_id
        )
...
def test_malformed_data_handling(alpaca_handler):
    ...
    malformed_data = {"prompt": [None], "output": ["Valid output"]}
...
def test_missing_data_handling(alpaca_handler):
    ...
    missing_data = {"prompt": ["Valid prompt"], "output": [None]}
