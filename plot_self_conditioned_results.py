#!/usr/bin/env python3
"""
Script to automatically plot the latest self-conditioned experiment results.

This script:
1. Finds the most recent self-conditioned experiment results file
2. Generates comprehensive visualizations
3. Saves plots to the current directory
4. Prints summary statistics

Usage:
    python plot_self_conditioned_results.py [results_file]
...
from dendritic.experiments.self_conditioned.visualization import (
    plot_loss_curves,
...
def find_latest_results_file(results_dir="results/self_conditioned_experiments"):
    """
    Find the most recent self-conditioned experiment results file.
...
def plot_results(results_file, output_prefix="self_conditioned"):
    """
    ...
    try:
        ...
        print("2. Generating training time comparison plot...")
        ...
        # 3. Generate and print summary statistics
...
def main():
    parser = argparse.ArgumentParser(
        description="Plot self-conditioned experiment results",
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog="""
Examples:
  %(prog)s                         # Plot latest results automatically
  %(prog)s results/self_conditioned_experiments/20260128_093429_results.json
  %(prog)s --prefix my_experiment  # Custom output prefix
    ...
    parser.add_argument(
        "--prefix",
        default="self_conditioned",
        help="Prefix for output files (default: 'self_conditioned')",
    )
    parser.add_argument(
        "--dir",
        default="results/self_conditioned_experiments",
        help="Results directory to search (default: 'results/self_conditioned_experiments')",
    )
    ...
    if success:
        print("\n✅ Successfully plotted self-conditioned experiment results!")
        sys.exit(0)
    else:
