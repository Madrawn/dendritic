...
def plot_training_time_comparison(
    ...
    if training_times[0] > 0:
        ...
        change_text = f"Slower by {abs(relative_change):.1f}%" if relative_change > 0 else f"Faster by {abs(relative_change):.1f}%"
...
def _plot_token_loss_comparison(results: ConfidenceExperimentResults, ax: Axes):
    ...
    if std_token_losses and conf_token_losses:
        ...
        ax.text(0.5, 0.5, 'No token loss data available', 
...
def _plot_confidence_loss(results: ConfidenceExperimentResults, ax: Axes):
    ...
    if confidence_losses:
        ...
        if len(confidence_losses) > 100:
            ...
            ax.plot(steps[window-1:], rolling_avg, color='darkred', linewidth=2, label=f'{window}-step avg')
            ax.legend()
    else:
        ax.text(0.5, 0.5, 'No confidence loss data available', 
                ha='center', va='center', transform=ax.transAxes)
        ax.set_title('Confidence Loss Over Time')
...
def _plot_eval_loss_comparison(results: ConfidenceExperimentResults, ax: Axes):
    ...
    for seed, result_list in results.standard_model_results.items():
        for result in result_list:
            if result.loss_history:
                ...
                eval_losses = [h.get("eval_loss", 0) for h in result.loss_history]
    ...
    if std_eval_losses and conf_eval_losses:
        ...
        ax.text(0.5, 0.5, 'No evaluation loss data available', 
                ha='center', va='center', transform=ax.transAxes)
        ax.set_title('Evaluation Loss Comparison')
...
def _plot_calibration_scatter(results: ConfidenceExperimentResults, ax: Axes):
    ...
    for seed, result_list in results.confidence_model_results.items():
        for result in result_list:
            if result.confidence_predictions and result.actual_future_losses:
                ...
                min_len = min(len(result.confidence_predictions), len(result.actual_future_losses))
    ...
    if confidence_predictions and actual_future_losses:
        ...
        x_ideal = np.linspace(min(confidence_predictions), max(confidence_predictions), 100)
...
def _plot_binned_calibration(results: ConfidenceExperimentResults, ax: Axes):
    ...
    if confidence_predictions and actual_future_losses:
        # Bin the data
        n_bins = 10
        bins = np.linspace(min(confidence_predictions), max(confidence_predictions), n_bins + 1)
        ...
        for i in range(n_bins):
            mask = (np.array(confidence_predictions) >= bins[i]) & (np.array(confidence_predictions) < bins[i+1])
            if np.any(mask):
        ...
        if bin_centers:
            ax.errorbar(bin_centers, bin_avg_losses, yerr=bin_std_losses, 
                       fmt='o-', capsize=5, label='Binned Calibration')
