...
class CohortLRScheduler:
    def __init__(
        self, min_mult=0.5, max_mult=1.0, sharpness=1.0, seed=42, device="cpu"
    ):
        """
        Args:
    ...
    def step(self):
        self.current_phase = (
            torch.rand(1, generator=self.rng, device=self.current_phase.device)
            * self.phase_range
        )
        self.step_count += 1
    ...
    def get_multipliers(self, layer, layer_size, device):
        if (
            layer not in self.permutations
        ):  # Fixed bug: checking 'layer' key instead of 'layer_size'
