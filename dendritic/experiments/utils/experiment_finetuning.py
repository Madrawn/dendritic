...
def calculate_dendritic_trainable_params(config: FinetuningConfig) -> int:
    ...
    for layer_name in config.target_layers:
        if layer_name in layer_dims or layer_name.replace("mlp.", "") in layer_dims:
            ...
            key = (
                layer_name
                if layer_name in layer_dims
                else layer_name.replace("mlp.", "")
            )
...
def calculate_matching_lora_rank(
    dendritic_params: int, config: FinetuningConfig
) -> int:
...
def setup_dendritic_stack_model(
    config: FinetuningConfig, device: str
) -> GPT2LMHeadModel:
...
def setup_lora_model(config: FinetuningConfig, lora_rank: int, device: str) -> Any:
...
def evaluate_finetuning(
    model: nn.Module, dataloader: DataLoader, max_batches: Optional[int], device: str
) -> float:
...
def run_finetuning_experiment(
    ...
    matched_params = {
        ...
        "dendritic_stack": (
            dendritic_stack_results[0].trainable_params
            if dendritic_stack_results
            else 0
        ),
...
def analyze_finetuning_results(
    ...
    cohens_ds = (
        np.mean(diff_ds) / np.std(diff_ds, ddof=1) if np.std(diff_ds, ddof=1) > 0 else 0
    )

    diff_dl = np.array(dendritic_ppl) - np.array(lora_ppl)
    cohens_dl = (
        np.mean(diff_dl) / np.std(diff_dl, ddof=1) if np.std(diff_dl, ddof=1) > 0 else 0
    )

    diff_sl = np.array(dendritic_stack_ppl) - np.array(lora_ppl)
    cohens_sl = (
        np.mean(diff_sl) / np.std(diff_sl, ddof=1) if np.std(diff_sl, ddof=1) > 0 else 0
    )

    # Improvement over baseline
    dendritic_improvement = 100 * (baseline_ppl - np.mean(dendritic_ppl)) / baseline_ppl
    dendritic_stack_improvement = (
        100 * (baseline_ppl - np.mean(dendritic_stack_ppl)) / baseline_ppl
    )
    ...
    return {
        ...
        "comparison_dendritic_vs_stack": {
            ...
            "stack_better_by_pct": 100
            * (np.mean(dendritic_ppl) - np.mean(dendritic_stack_ppl))
            / np.mean(dendritic_ppl),
        ...
        "comparison_dendritic_vs_lora": {
            ...
            "dendritic_better_by_pct": 100
            * (np.mean(lora_ppl) - np.mean(dendritic_ppl))
            / np.mean(lora_ppl),
        ...
        "comparison_stack_vs_lora": {
            ...
            "stack_better_by_pct": 100
            * (np.mean(lora_ppl) - np.mean(dendritic_stack_ppl))
            / np.mean(lora_ppl),
...
def save_finetuning_results(
    results: FinetuningExperimentResults, output_dir: Path
) -> None:
...
def print_finetuning_summary(results: FinetuningExperimentResults) -> None:
    ...
    print("\n--- Final Perplexity ---")
    print(
        f"Dendritic:       {analysis['dendritic']['final_ppl_mean']:.2f} ± {analysis['dendritic']['final_ppl_std']:.2f}"
    )
    print(
        f"Dendritic Stack: {analysis['dendritic_stack']['final_ppl_mean']:.2f} ± {analysis['dendritic_stack']['final_ppl_std']:.2f}"
    )
    print(
        f"LoRA:            {analysis['lora']['final_ppl_mean']:.2f} ± {analysis['lora']['final_ppl_std']:.2f}"
    )

    print("\n--- Improvement over Baseline ---")
    print(
        f"Dendritic:       {analysis['dendritic']['improvement_over_baseline_pct']:.1f}%"
    )
    print(
        f"Dendritic Stack: {analysis['dendritic_stack']['improvement_over_baseline_pct']:.1f}%"
    )
    ...
    def print_comparison(name: str, comp: Dict[str, Any]) -> None:
        ...
        print(
            f"Paired t-test: t={comp['paired_ttest']['t_statistic']:.3f}, p={comp['paired_ttest']['p_value']:.4f}"
        )
    ...
    print_comparison(
        "Dendritic vs Dendritic Stack", analysis["comparison_dendritic_vs_stack"]
    )
