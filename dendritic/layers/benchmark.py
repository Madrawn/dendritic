...
def test_structure_responsiveness(
    ...
    for poly_rank in [8, 6, 4, 2]:
        for poly_degree in [3, 4]:
            models_struct[f"DendriticStack r={poly_rank} d={poly_degree}"] = functools.partial(
                DendriticStack, input_dim, output_dim, poly_rank=poly_rank, poly_degree=poly_degree
            )
...
def assess_cubic_feature_contributions(
    ...
    for poly_rank in [8, 6, 4, 2]:
        for poly_degree in [3, 4]:
            models_order[f"DendriticStack r={poly_rank} d={poly_degree}"] = functools.partial(
                DendriticStack, input_dim, output_dim, poly_rank=poly_rank, poly_degree=poly_degree
            )
...
def diagnostic_benchmark(n_batches: int = 5, batch_size: int = 64) -> None:
    """
    ...
    # Test 1: Rank scaling
    evaluate_rank_scaling(input_dim, output_dim, n_batches, batch_size)
    
    # Test 2: Diagonal vs Cross structure
    test_structure_responsiveness(input_dim, output_dim, n_batches, batch_size, test_poly_rank)

    # Test 3: Cubic necessity
    assess_cubic_feature_contributions(
        input_dim, output_dim, n_batches, batch_size, test_poly_rank
    )
