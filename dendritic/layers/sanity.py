...
def test_shapes() -> None:
    ...
    y = layer(x)
    assert y.shape == (TEST_BATCH_SIZE, DEFAULT_OUTPUT_DIM), \
    ...
    assert y_stack.shape == (TEST_BATCH_SIZE, DEFAULT_OUTPUT_DIM), \
...
def test_gradient_flow() -> None:
    ...
    assert layer.w1.grad is not None, "Gradient missing for w1"
    assert layer.scale.grad is not None, "Gradient missing for scale"
    ...
    for w in stack.projections:
        assert w.grad is not None, f"Gradient missing for projection {w}"
...
def test_parameter_count() -> Tuple[int, int]:
    ...
    if layer.diag_rank is not None and isinstance(layer.diag_rank, int) and layer.diag_rank > 0:
        expected = (
            (DEFAULT_INPUT_DIM * DEFAULT_OUTPUT_DIM + DEFAULT_OUTPUT_DIM)
...
def _test() -> None:
    ...
    print("\nAll tests passed!")
    print(f"DendriticLayer({DEFAULT_INPUT_DIM}, {DEFAULT_OUTPUT_DIM}, poly_rank={DEFAULT_POLY_RANK}): {layer_params:,} parameters")
    print(f"DendriticStack({DEFAULT_INPUT_DIM}, {DEFAULT_OUTPUT_DIM}, poly_rank={DEFAULT_POLY_RANK}): {stack_params:,} parameters")
    print(f"  vs nn.Linear({DEFAULT_INPUT_DIM}, {DEFAULT_OUTPUT_DIM}): {linear_params:,} parameters")
