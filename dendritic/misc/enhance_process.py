...
def train_model(
    ...
    tracker = ExperimentTracker(
        method_name="dendritic_finetune", params=config.__dict__
    )
    ...
    for step in progress_bar:
        ...
        if (step + 1) % config.gradient_accumulation_steps == 0:
            ...
            # Update progress bar
            progress_bar.set_postfix({"avg_loss": f"{accumulated_loss:.4f}"})
            accumulated_loss = 0.0
        ...
        if (step + 1) % config.eval_interval == 0:
            best_eval_ppl = run_evaluation(
                model, eval_dataloader, step, best_eval_ppl, tokenizer, device, tracker
            )
            model.train()

    return {"tracker": tracker, "best_eval_ppl": best_eval_ppl}


def log_training_progress(
    model: GPT2LMHeadModel, step: int, tracker: ExperimentTracker
) -> None:
...
def main():
    ...
    eval_dataloader = DataLoader(
        data["eval"], batch_size=config.batch_size, shuffle=False
    )
    ...
    training_results = train_model(
        model_dendritic, train_dataloader, eval_dataloader, tokenizer, config, device
    )
