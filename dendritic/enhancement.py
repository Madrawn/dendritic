...
class LayerConversionStats:
    ...
    def trainable_percentage(self) -> float:
        return (
            (self.trainable_params / self.dendritic_params) * 100
            if self.dendritic_params > 0
            else 0.0
        )
...
def _create_dendritic_layer(
    ...
    _initialize_dendritic_layer(dendritic, module, is_conv1d)
...
def _initialize_dendritic_layer(
    ...
    with torch.no_grad():
        if (
            ...
            and dendritic.__class__.__name__ == "DendriticStack"
        ):
            # Handle weight source with proper typing
            weight_tensor = cast(Tensor, original_module.weight)
            src_weight = (
                weight_tensor if not is_conv1d else weight_tensor.t().contiguous()
            )
            ...
            if hasattr(dendritic, "linear") and hasattr(original_module, "weight"):
                ...
                weight_tensor = cast(Tensor, original_module.weight)
                src_weight = (
                    weight_tensor.t().contiguous() if is_conv1d else weight_tensor
                )
                ...
                # Handle bias if exists
                if (
                    hasattr(original_module, "bias")
                    and original_module.bias is not None
                ):
                    bias_tensor = cast(Tensor, original_module.bias)
...
def _get_module_parent_and_name(
    ...
    **kwargs,
    ...
    for name, module in model.named_modules():
        ...
        try:
            ...
            # 1) Make all dendritic params trainable (adapter params)
            for pname, p in dendritic.named_parameters():
                p.requires_grad = True
                dendritic_param_ids.add(id(p))

            # 2) Optionally freeze the identity path so only adapter learns
            if freeze_linear:
                _freeze_linear_pathway(
                    dendritic
                )  # sets linear/base_linear .requires_grad=False
        ...
        with torch.no_grad():
            ...
            if hasattr(module, "w1"):
                try:
                    ...
                    logger.warning(
                        f"Failed to calculate effective rank for {name}: {str(e)}"
                    )
...
def extract_dendritic_state(model: nn.Module) -> Dict[str, Any]:
    ...
    dendritic_weights = {
        k: v.cpu().clone() for k, v in full_state.items() if k in trainable_keys
    }

    return {
        "dendritic_config": model.dendritic_config,
        "state_dict": dendritic_weights,
        "_metadata": {"version": "2.0", "type": "dendritic_bundle"},
    }


def apply_dendritic_state(
    base_model: nn.Module, state_payload: Dict[str, Any]
) -> nn.Module:
    """
    ...
    if unexpected:
        print(
            f"WARNING: {len(unexpected)} unexpected keys found while loading dendritic state."
        )
...
if __name__ == "__main__":
    ...
    new_enhanced = apply_dendritic_state(model, state)
